...
def exportar_consultas_a_excel(fecha_str: str):
    ...
    try:
        ...
        # 2. Lógica de consolidación con Pandas
        def obtener_registro_definitivo(grupo):
            # Ordena los registros del grupo del más nuevo al más viejo
            grupo = grupo.sort_values(by='fecha_consulta', ascending=False)
            
            if len(grupo) == 1:
                return grupo.iloc[0] # Si solo hay uno, lo devolvemos
            
            registro_mas_nuevo = grupo.iloc[0]
            registro_anterior = grupo.iloc[1]
            
            if registro_mas_nuevo['estado_consulta'] == 'Exitoso':
                return registro_mas_nuevo # El más nuevo es exitoso, lo usamos
            else:
                return registro_anterior # El más nuevo falló, usamos el de respaldo
